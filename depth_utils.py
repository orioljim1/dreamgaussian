...
def normalize_depth5(depth, power=1.6):
    # Normalize the depth values to the range [0, 1]
    normalized_depth = (depth - depth.min()) / (depth.max() - depth.min())
    
    # Apply a stronger non-linear transformation to emphasize smaller values
    transformed_depth = torch.exp(normalized_depth * power) - 1
    
    # Normalize again to ensure the values are in the range [0, 1]
    transformed_depth = (transformed_depth - transformed_depth.min()) / (transformed_depth.max() - transformed_depth.min())
